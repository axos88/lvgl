...
the draw pass. <ApiLink name="lv_barcode_get_render_valid" /> covers those:

```c
lv_obj_set_height(barcode, 0);   /* leaves no room for a horizontal barcode */
if(!lv_barcode_get_render_valid(barcode)) {
    /* the bitmap is blank; give the object a height */
}
```

It is also `false` before any data is set, and `true` again after a success. A failure is
