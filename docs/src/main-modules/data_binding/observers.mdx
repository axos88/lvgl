...
<Callout type="warning">
An Observer's mapper is shaped differently from a Subject's. It has **no `input`
parameter**, and `out` is the Observer's *own output slot* — the value it last pushed to
the Widget, not the Subject's value. Read the observed value with
<ApiLink name="lv_observer_get_subject" display="lv_observer_get_subject()" /> and the
matching getter; `*out` exists only so the mapper can answer "did my output change?".
</Callout>
