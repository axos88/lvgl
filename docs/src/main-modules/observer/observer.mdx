...
{/*
  NOTE: this page is not listed in ../meta.json, so it is not part of the built docs.
  The live Observer documentation is under main-modules/data_binding/. This file is kept
  only so its content is not lost; parts of it describe the pre-mapper API and the
  removed Group Subject type.
*/}
...
| Field | Description |
|-------|-------------|
| `Subjects` | (in global memory or heap) are "logic packages", each containing the value being "observed" and its type (integer (`int32_t`), a string, a pointer, an <ApiLink name="lv_color_t" />, a `float`, or nothing at all); |
...
2. Subscribe to an aggregate Subject (one whose mapper reads several other Subjects)
    to get notified when any of those Subjects' values change.
...
| `None` (aggregates other Subjects) | `LV_SUBJECT_TYPE_NONE` |

A new Subject starts from a neutral value (`0`, `NULL`, black, an empty string, an
no value for `LV_SUBJECT_TYPE_NONE`), so set the initial value with the matching `lv_subject_set_...()`
function afterwards. Two types need one extra call before they can hold a value:

| Type | Extra setup |
|------|-------------|
| `String` | void <ApiLink name="lv_subject_set_string_buffer_static" display="lv_subject_set_string_buffer_static(subject, buf, buf_size)" /> |
| `None` | void <ApiLink name="lv_subject_set_none_mapper" display="lv_subject_set_none_mapper(subject, mapper)" /> |

The string buffers are used in place, not copied, so they must out-live the Subject.
...
A Subject does not keep its previous value. A mapper is handed the previous stored
value, and an Observer that needs one remembers it itself.
...
A Subject has no built-in range. Attach a clamping mapper with
<ApiLink name="lv_subject_set_int_mapper" display="lv_subject_set_int_mapper()" />
(or the float variant) instead:
...
See "Mappers" under Data binding for the full picture.
...
To handle this, create a Subject of type `LV_SUBJECT_TYPE_NONE`, which has no value of
its own, and give it a mapper that reads all three. Reading them is what makes them its
dependencies.

```c
static bool all_mapper(lv_subject_t * subject)
{
    LV_UNUSED(subject);
    (void)lv_subject_get_int(subject_1);
    (void)lv_subject_get_int(subject_2);
    (void)lv_subject_get_int(subject_3);
    return true;   /* notify whenever any of them changed */
}

subject_all = lv_subject_create(LV_SUBJECT_TYPE_NONE);
lv_subject_set_none_mapper(subject_all, all_mapper);
```

Observers are then added to `subject_all` in the usual way. A change to any of the
Subjects its mapper reads triggers a notification to all of its Observers.
...
lv_subject_t * subject_all;   // Aggregate that connects the above 3 Subjects
...
static void all_observer_cb(lv_observer_t * observer, lv_subject_t * subject)
{
    ...
    LV_UNUSED(subject);
    /* Read the source Subjects directly. */
...
set by the min/max values of the event. A clamping mapper on the subject, if it has
one, applies on top of whatever the event writes.
