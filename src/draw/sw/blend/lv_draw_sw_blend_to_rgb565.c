...
void LV_ATTRIBUTE_FAST_MEM lv_draw_sw_blend_color_to_rgb565(lv_draw_sw_blend_fill_dsc_t * dsc)
{
    ...
    /*Opacity only*/
        ...
        if(LV_RESULT_INVALID == LV_DRAW_SW_COLOR_BLEND_TO_RGB565_WITH_OPA(dsc)) {
             ...
             *pixels then cost a compare instead of a mix. Only the paired loop reads it.*/
            uint16_t last_dest_color = 0;
            uint32_t last_res32_color = 0;
            if(w > 2) {
                last_dest_color = dest_buf_u16[0];
                last_res32_color = lv_color_16_16_mix(color16, last_dest_color, opa);
