...
static void spinbox_value_observer_cb(lv_observer_t * observer, lv_subject_t * subject)
{
    ...
    LV_ASSERT(subject->type == LV_SUBJECT_TYPE_INT || subject->type == LV_SUBJECT_TYPE_FLOAT);
    if(subject->type == LV_SUBJECT_TYPE_INT) {
        lv_spinbox_set_value(lv_observer_get_target_obj(observer), subject->value.num);
    }
#if LV_USE_FLOAT
    else {
        lv_spinbox_set_value(lv_observer_get_target_obj(observer), (int32_t)subject->value.float_v);
    }
#endif
}
